...
初始化应用设备的网络类型，设备类型的改变都要产生一个事件—ZDO_STATE_CHANGE，从面理解为 ZDO 状态发生了改变。所以在设备初始化的时候一定要把它初始化为什么状态都�?有。那么它就要去检测整个环境，看是否能重新建立或者加入存在的网络。但是有一种情�?例外，就是当 NV_RESTORE 被设置的候（NV_RESTORE 是把信息保存在非易失存储器中），那么
当设备断电或者某种意外重启时，由于网络状态存储在非易失存储器中，那么此时就只需恢复其网络状态，而不需要重新建立或者加入网络了.**这里需要设�?NV_RESTORE 宏定�?*�?*/
